...
function BinaryHeap () {
  ...
  this._getParentIdx = childIdx => Math.floor( (childIdx - 1) / 2 );
  this._childrenIdxs = parentIdx => [parentIdx * 2 + 1, parentIdx * 2 + 2];
  this._swap = (arr, i, j) => {
    let temp = arr[i];
    arr[i] = arr[j];
    arr[j] = temp;
  };
  this._swapCompare = (arr, idx, isChild = true) => {
    if (isChild) {
      let j = this._getParentIdx(idx);
      if (arr[j] !== undefined && this._compare(arr[idx], arr[j])) {
        // if child less than parent swap up recursively
        this._swap(arr, idx, j);
        this._swapCompare(arr, j, isChild);
      }
    } else {
      let j = this._childrenIdxs(idx);
      let p = arr[idx];
      let c1 = arr[j[0]];
      let c2 = arr[j[1]]
      if (c1 !== undefined && this._compare(c1, p) && c1 < c2) {
        // if parent greater than child 1 swap down recursively
        this._swap(arr, idx, j[0]);
        this._swapCompare(arr, j[0], isChild);
      } else if (c2 !== undefined && this._compare(c2, p) && c2 < c1) {
        this._swap(arr, idx, j[1]);
        this._swapCompare(arr, j[1], isChild);
      }
    }
  }
}
...
BinaryHeap.prototype.insert = function (value) {
  this._heap.push(value); // Insert child at end
  this._swapCompare(this._heap, this._heap.length - 1, true); // Bubble child up
}

BinaryHeap.prototype.removeRoot = function () {
  this._swap(this._heap, 0, this._heap.length - 1); // Swap root and last
  // console.log(this._heap[0] + ' ' + this._heap[this._heap.length - 1]);
  let minMax = this._heap.pop(); // Remove last which was formerly root
  this._swapCompare(this._heap, 0, false); // Bubble parent down
  return minMax;
}
