...
Tree.prototype.DFSelect = function(filter, acc = [], depth = 0) {
  filter(this.value, depth) && acc.push(this.value);
  this.children.forEach(tree => tree.DFSelect(filter, acc, depth+1));
  return acc;
};
...
(()=>{
  ...
  // Works for empty
  // console.log(new Tree(null).DFSelect(x => true))
})();
