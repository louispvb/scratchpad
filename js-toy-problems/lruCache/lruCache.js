...
LRUCache.prototype.get = function (key) {
  if (this.hashmap[key]) {
    // Move list node to end of array where I'm keeping most recently accessed
    let listNode = this.hashmap[key]
    this.list.moveToEnd(listNode)
    return listNode.val.val;
  }
  return null; // Return null when the value is not found
};

LRUCache.prototype.set = function (key, val) {
  if (this.size() >= this.limit) {
    // If over limit removed from front of list where I'm keeping oldest accessed
    let removed = this.list.shift();
    delete this.hashmap[removed.key]; // Remove the mapping from the hashmap too
  }

  let newItem = new LRUCacheItem(val, key);

  // Store list nodes in the hashmap so when we retrieve from it later we can get the 
  // list node that we can pass to the list.moveToEnd function that expects a list node
  let node = this.list.push(newItem); 
