 ...
 *     There is an algorithm that uses O(N) time and O(1) space (i.e., an in-place shuffle).
 ...
 I remain unbiased because every time I pick an element to swap, it's not any of the previously
 already shuffled elements.

Space complexity is O(1), it does in place linear shuffling.
...
var shuffleDeck = function(deck) {
  let getRandomIdx = max => Math.floor(Math.random() * (max + 1));
  for (let i = deck.length - 1; i >= 0; i--) {
    let j = getRandomIdx(i);
    let tmp = deck[i];
    deck[i] = deck[j];
    deck[j] = tmp;
  }
